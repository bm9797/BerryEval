...
Create empty `__init__.py` so the benchmarks directory is importable as a Python package (needed for `from benchmarks.conftest import ...` in test files):
```python
"""Benchmark suite for BerryEval metrics performance."""
```
