...
- Use -std=c99 for portability
- The `name` must be exactly `berryeval.metrics._native` to match the import in __init__.py
- On Windows, setuptools handles MSVC automatically (no -O2/-std=c99 needed — setuptools strips unknown flags for MSVC). Add platform check:
...
- Count hits: iterate j from 0 to k-1, if is_in_relevant(ret_row[j], rel_row, n_relevant) then hits++
- out[i] = (float)hits / (float)n_rel;

**berry_precision_at_k implementation:**
Same as recall but: out[i] = (float)hits / (float)k;
